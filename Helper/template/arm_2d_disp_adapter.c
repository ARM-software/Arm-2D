...
#if __DISP%Instance%_CFG_COLOR_SOLUTION__ != 1              /* as long as it is not monochrome */
    ...
    && __DISP%Instance%_CFG_CONSOLE_DISPALY_TIME__ != 0xFFFFFFFF
        if (DISP%Instance%_CONSOLE.bShowConsole) {
            if (arm_2d_helper_is_time_out(__DISP%Instance%_CFG_CONSOLE_DISPALY_TIME__, &DISP%Instance%_CONSOLE.lTimestamp)) {
                ...
                DISP%Instance%_CONSOLE.chOpacity = 0;
            } else {
...
static void __user_scene_player_init(void)
{
...
#endif
#if __DISP%Instance%_CFG_DISABLE_DYNAMIC_PFB__
...
#endif
        .tAntiNoiseScanSize = {
            .iWidth = __DISP%Instance%_CFG_PFB_ANS_WIDTH__,
            .iHeight = __DISP%Instance%_CFG_PFB_ANS_HEIGHT__,
        },
    ...
    extern arm_2d_helper_dma_copy_handler_t __disp_adapter%Instance%_request_dma_copy;
        ...
        arm_2d_helper_3fb_cfg_t tCFG = {
            .tScreenSize = {
#if     __DISP%Instance%_CFG_ROTATE_SCREEN__ == 1\
    ||  __DISP%Instance%_CFG_ROTATE_SCREEN__ == 3
                __DISP%Instance%_CFG_SCEEN_HEIGHT__,
                __DISP%Instance%_CFG_SCEEN_WIDTH__,
#else
                __DISP%Instance%_CFG_SCEEN_WIDTH__,
...
void disp_adapter%Instance%_navigator_init(void)
{
    ...
    arm_2d_helper_pfb_dependency_t tDependency = {
        .Navigation.evtOnDrawing.fnHandler = &__disp_adapter%Instance%_draw_navigation,
        .Navigation.evtOnDrawing.pTarget = &DISP%Instance%_ADAPTER,
        .Navigation.ptDirtyRegion = (arm_2d_region_list_item_t *)s_tNavDirtyRegionList,
    }; 

    arm_2d_helper_pfb_update_dependency(&DISP%Instance%_ADAPTER.use_as__arm_2d_helper_pfb_t, 
...
ARM_PT_BEGIN(DISP%Instance%_ADAPTER.chPT)

    arm_2d_scene_t *ptScene = disp_adapter%Instance%_get_current_scene();
...
ARM_PT_YIELD((arm_fsm_rt_t)ARM_2D_RT_PFB_USER_DRAW);
        ...
        ptScene = disp_adapter%Instance%_get_current_scene();
    }
...
__WEAK 
void __disp_adapter%Instance%_vres_asset_2dcopy( uintptr_t pObj,
                                            ...
                                            arm_2d_region_t *ptRegion)
{
     ...
     *       __disp_adapter%Instance%_vres_asset_hint_depose also.
     *       
     */
    return false;
}
                                            ...
                                            arm_2d_region_t *ptRegion)
{
    ...
    do {
        ...
        __disp_adapter%Instance%_vres_asset_2dcopy(  pObj, 
                                            ptVRES, 
                                            ptRegion, 
                                            pSrc, 
                                            pDes, 
