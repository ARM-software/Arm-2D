...
static Vector2 gravityForce = { 0.0f,  9.81f * 65536.0f };              // Physics world gravity force
...
#ifndef PHYSAC_GET_CURRENT_TIME
static double GetCurrTime(void);                                                                            // Get current time measure in milliseconds
#   define PHYSAC_GET_CURRENT_TIME()    GetCurrTime()
#endif
...
{
    gravityForce.q16X = reinterpret_q16_f32(x);
    gravityForce.q16Y = reinterpret_q16_f32(y);
}
...
PHYSACDEF PhysicsBody CreatePhysicsBodyRectangle(Vector2 pos, float width, float height, float density)
{
    ...
    if (newId != -1)
    {
        ...
        for (int i = 0; i < newBody->shape.vertexData.vertexCount; i++)
        {
            ...
            q16_t D = reinterpret_q16_f32( MathCrossVector2(p1, p2));
            q16_t triangleArea = div_n_q16(D, 2);

            area += triangleArea;

            // Use area to weight the centroid average, not just vertex position
            center.q16X += mul_q16( mul_q16( triangleArea, reinterpret_q16_f32(PHYSAC_K)), (p1.q16X + p2.q16X));
            center.q16Y += mul_q16( mul_q16( triangleArea, reinterpret_q16_f32(PHYSAC_K)), (p1.q16Y + p2.q16Y));

            float intx2 = reinterpret_f32_q16( mul_q16(p1.q16X, p1.q16X) + mul_q16(p2.q16X, p1.q16X) + mul_q16( p2.q16X, p2.q16X));
            float inty2 = reinterpret_f32_q16( mul_q16(p1.q16Y, p1.q16Y) + mul_q16(p2.q16Y, p1.q16Y) + mul_q16( p2.q16Y, p2.q16Y));
            inertia += (0.25f*PHYSAC_K* reinterpret_f32_q16(D))*(intx2 + inty2);
        }
...
PHYSACDEF void RunPhysicsStep(void)
{
    ...
    currentTime = PHYSAC_GET_CURRENT_TIME();
